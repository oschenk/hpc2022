 ...
 *                                                              *  
 * Purpose: A program to try MPI_Issend and MPI_Irecv.          *
 *                                                              *
 * Contents: C-Source                                           *
 *                                                              *
 ****************************************************************/
...
int main(int argc, char* argv[])
{
	...
	for (int i = 0, sent_buf = my_rank, recv_buf; i < size; i++, sent_buf = recv_buf) 
	{
		...
		MPI_ERR_CHECK(MPI_Irecv(&recv_buf, 1, MPI_INT, left, to_right,
			MPI_COMM_WORLD, &arr_request[1]));

		MPI_Status arr_status[2];
		MPI_ERR_CHECK(MPI_Waitall(2, arr_request, arr_status));
